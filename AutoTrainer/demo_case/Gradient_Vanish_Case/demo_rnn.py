...
def get_dataset(dataset,labels=2):
    data_name=dataset.split('_')[0]
    data = importlib.import_module('{}'.format(data_name.lower()), package='data')
    (x, y), (x_val, y_val) = data.load_data(10000)
    preprocess_func = data.preprocess
    dataset={}
    dataset['x']=preprocess_func(x,300)
    dataset['x_val']=preprocess_func(x_val,300)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path','-mp',default='./model_rnn.h5', help='model path')
    parser.add_argument('--config_path', '-cp',default='./config_rnn.pkl', help='training configurations path') 
